...
main()
{
    ...
    modbus_master_read_input_registers(1, MODBUS_REGISTER_CARDS_COUNT_IDX, 1)
    new val_l = (registers_values[0] >> 8)
    new val_h = (registers_values[0] << 8)
    SetVar(globCardsCount, ((val_h + val_l) % 256))
}
...
modbus_recieve_data(reg_count)
{
    ...
    for (new i = 0; i < reg_count; i++) {
        ...
        Diagnostics("COMMAND: register %d - %d (0x%X)", i, val, val)
    }
}
