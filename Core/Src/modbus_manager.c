...
//const uint16_t MODBUS_REGISTER_CF_ID_IDX        = ((uint16_t)0);
//const uint16_t MODBUS_REGISTER_DEVICE_ID_IDX    = ((uint16_t)MODBUS_REGISTER_CF_ID_IDX + MODBUS_U8_ARRAY_REG_COUNT(settings.cf_id));
//const uint16_t MODBUS_REGISTER_CARDS_IDX        = ((uint16_t)MODBUS_REGISTER_DEVICE_ID_IDX + MODBUS_U8_ARRAY_REG_COUNT(settings.device_id));
//const uint16_t MODBUS_REGISTER_CARDS_VALUES_IDX = ((uint16_t)MODBUS_REGISTER_CARDS_IDX + MODBUS_U8_ARRAY_REG_COUNT(settings.cards));
//const uint16_t MODBUS_REGISTER_LOG_ID_IDX       = ((uint16_t)MODBUS_REGISTER_CARDS_VALUES_IDX + MODBUS_U8_ARRAY_REG_COUNT(settings.cards_limits));
//const uint16_t MODBUS_REGISTER_YEAR_IDX         = ((uint16_t)MODBUS_REGISTER_LOG_ID_IDX + MODBUS_U8_ARRAY_REG_COUNT(settings.log_id));
//const uint16_t MODBUS_REGISTER_MONTH_IDX        = ((uint16_t)MODBUS_REGISTER_YEAR_IDX + MODBUS_U8_ARRAY_REG_COUNT(uint8_t));
//const uint16_t MODBUS_REGISTER_DAY_IDX          = ((uint16_t)MODBUS_REGISTER_MONTH_IDX + MODBUS_U8_ARRAY_REG_COUNT(uint8_t));
//const uint16_t MODBUS_REGISTER_HOUR_IDX         = ((uint16_t)MODBUS_REGISTER_DAY_IDX + MODBUS_U8_ARRAY_REG_COUNT(uint8_t));
//const uint16_t MODBUS_REGISTER_MINUTE_IDX       = ((uint16_t)MODBUS_REGISTER_HOUR_IDX + MODBUS_U8_ARRAY_REG_COUNT(uint8_t));
//const uint16_t MODBUS_REGISTER_SECOND_IDX       = ((uint16_t)MODBUS_REGISTER_MINUTE_IDX + MODBUS_U8_ARRAY_REG_COUNT(uint8_t));
//
//const uint16_t MODBUS_REGISTER_CARDS_COUNT_IDX  = ((uint16_t)0);
//const uint16_t MODBUS_REGISTER_RCRD_ID_IDX      = ((uint16_t)MODBUS_REGISTER_CARDS_COUNT_IDX + MODBUS_U8_ARRAY_REG_COUNT(uint16_t));
//const uint16_t MODBUS_REGISTER_TIME_IDX         = ((uint16_t)MODBUS_REGISTER_RCRD_ID_IDX + MODBUS_U8_ARRAY_REG_COUNT(log_record.id));
//const uint16_t MODBUS_REGISTER_RCRD_CF_ID_IDX   = ((uint16_t)MODBUS_REGISTER_TIME_IDX + MODBUS_U8_ARRAY_REG_COUNT(log_record.time));
//const uint16_t MODBUS_REGISTER_CARD_IDX         = ((uint16_t)MODBUS_REGISTER_RCRD_CF_ID_IDX + MODBUS_U8_ARRAY_REG_COUNT(log_record.cf_id));
//const uint16_t MODBUS_REGISTER_USED_LITERS_IDX  = ((uint16_t)MODBUS_REGISTER_CARD_IDX + MODBUS_U8_ARRAY_REG_COUNT(log_record.card));


typedef struct _modbus_manager_state_t {
    uint8_t      data[MODBUS_RESPONSE_MESSAGE_SIZE];
    uint8_t      length;
...
void _modbus_manager_response_data_handler(uint8_t* data, uint32_t len);
...
//        tmp_settings.device_id <<= 16;
//        tmp_settings.device_id  |= modbus_slave_get_register_value(MODBUS_REGISTER_ANALOG_OUTPUT_HOLDING_REGISTERS, MODBUS_REGISTER_DEVICE_ID_IDX + i);
//    }
//
...
//    }
//
//    for (uint16_t i = 0; i < MODBUS_U8_ARRAY_REG_COUNT(tmp_settings.cards_limits); i+=sizeof(uint16_t)) {
//        for (uint16_t j = 0; j < sizeof(*tmp_settings.cards_limits); j++) {
//            tmp_settings.cards_limits[i / 2] <<= 16;
//            tmp_settings.cards_limits[i / 2] |= modbus_slave_get_register_value(MODBUS_REGISTER_ANALOG_OUTPUT_HOLDING_REGISTERS, MODBUS_REGISTER_CARDS_VALUES_IDX + i + j);
//        }
//    }
//
...
//    settings_update_device_id(tmp_settings.device_id);
...
//		STRUCT_MEMBER_NOWARN_UNSAFE(struct _settings_t, &tmp_settings, cards_limits),
//		__arr_len(settings.cards_limits)
//	);
...
//	modbus_slave_set_register_value(
//		MODBUS_REGISTER_ANALOG_OUTPUT_HOLDING_REGISTERS,
//		MODBUS_REGISTER_DEVICE_ID_IDX,
//		settings.device_id >> 16
//	);
//	modbus_slave_set_register_value(
//		MODBUS_REGISTER_ANALOG_OUTPUT_HOLDING_REGISTERS,
//		MODBUS_REGISTER_DEVICE_ID_IDX + 1,
//		settings.device_id
//	);
//
...
//    }
//
//    for (uint16_t i = 0; i < __arr_len(settings.cards_limits); i++) {
//        modbus_slave_set_register_value(
//            MODBUS_REGISTER_ANALOG_OUTPUT_HOLDING_REGISTERS,
//            MODBUS_REGISTER_CARDS_IDX + 2 * i,
//            settings.cards_limits[i] >> 16
//        );
//        modbus_slave_set_register_value(
//            MODBUS_REGISTER_ANALOG_OUTPUT_HOLDING_REGISTERS,
//            MODBUS_REGISTER_CARDS_IDX + 2 * i + 1,
//            settings.cards_limits[i]
//        );
//    }
//
//    modbus_slave_set_register_value(
...
//		MODBUS_REGISTER_CARDS_COUNT_IDX,
...
//    	uint16_t reg_val = ((uint16_t)tmp_record.time[2 * i]) << 8;
