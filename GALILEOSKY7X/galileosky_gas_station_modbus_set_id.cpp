...
main()
{
    ...
    new device_id = GetVar(device_id)
    if (device_id <= 0) {
    ...
    val_l += ((device_id & 0x000000FF) << 8)
    val_l += ((device_id & 0x0000FF00) >> 8)
	val_l = ((val_l) & 0xFFFF)
	new addr1 = MODBUS_REGISTER_DEVICE_ID_IDX
	Diagnostics("COMMAND: modbus write (%d) - %X", addr1, val_l) 
    modbus_master_preset_single_register(1, addr1, val_l)
    
    new val_h = 0x000000000;
    val_h += ((device_id & 0x00FF0000) << 8)
    val_h += ((device_id & 0xFF000000) >> 8)
	val_h = ((val_h >> 16) & 0xFFFF)
	new addr2 = MODBUS_REGISTER_DEVICE_ID_IDX + 1
