...
main()
{
    ...
    if (cards_count <= 0) {
        Diagnostics("COMMAND: error - card_count=%d", cards_count) 
    ...
    val_l += ((log_id & 0x000000FF) << 8)
    val_l += ((log_id & 0x0000FF00) >> 8)
    val_l = ((val_l) & 0xFFFF)
    new addr1 = (MODBUS_REGISTER_CARDS_IDX + 5 * cards_count)
    Diagnostics("COMMAND: modbus write (%d) - %X", addr1, val_l) 
    modbus_master_preset_single_register(1, addr1, val_l)
    
    new val_h = 0x000000000;
    val_h += ((log_id & 0x00FF0000) << 8)
    val_h += ((log_id & 0xFF000000) >> 8)
    val_h = ((val_h >> 16) & 0xFFFF)
    new addr2 = (MODBUS_REGISTER_CARDS_IDX + 5 * cards_count) + 1
    Diagnostics("COMMAND: modbus write (%d) - %X", addr2, val_h) 
    modbus_master_preset_single_register(1, addr2, val_h)
    
    // Get log id
    modbus_master_read_holding_registers(1, addr1, 2)
    new val = 0x00000000
