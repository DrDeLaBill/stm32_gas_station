...
#define MODBUS_REGISTER_RCRD_ID_IDX          (MODBUS_REGISTER_CARDS_COUNT_IDX + 1)
#define MODBUS_REGISTER_RCRD_TIME_IDX        (MODBUS_REGISTER_RCRD_ID_IDX + 2)
#define MODBUS_REGISTER_RCRD_CARD_IDX        (MODBUS_REGISTER_RCRD_TIME_IDX + 2)
#define MODBUS_REGISTER_RCRD_USED_LITERS_IDX (MODBUS_REGISTER_RCRD_CARD_IDX + 2)
...
main()
{
    ...
    new log_id = GetVar(globLogId)
    if (log_id < 0) {
        SetVar(globResult, SCRIPT_RESPONSE_ERROR_ARG)
        return
    }
	
    Diagnostics("COMMAND: get next log for current log_id=%d", log_id)
    
    modbus_master_read_input_registers(1, MODBUS_REGISTER_RCRD_ID_IDX, 2)
    new val = 0x00000000
    ...
    modbus_master_read_input_registers(1, MODBUS_REGISTER_RCRD_TIME_IDX, 6)
    val = 0x00000000
    val += ((registers_values[0] >> 8)  & 0x000000FF)
    val += ((registers_values[0] << 8)  & 0x0000FF00)
    val += ((registers_values[1] << 8)  & 0x00FF0000)
    val += ((registers_values[1] << 24) & 0xFF000000)
    SetVar(rcrd_time, val)
    
    modbus_master_read_input_registers(1, MODBUS_REGISTER_RCRD_CARD_IDX, 2)
    val = 0x00000000
    val += ((registers_values[0] >> 8)  & 0x000000FF)
    val += ((registers_values[0] << 8)  & 0x0000FF00)
    val += ((registers_values[1] << 8)  & 0x00FF0000)
    val += ((registers_values[1] << 24) & 0xFF000000)
    SetVar(rcrd_card, val)
    
    modbus_master_read_input_registers(1, MODBUS_REGISTER_RCRD_USED_LITERS_IDX, 2)
    val = 0x00000000
    val += ((registers_values[0] >> 8)  & 0x000000FF)
    val += ((registers_values[0] << 8)  & 0x0000FF00)
    val += ((registers_values[1] << 8)  & 0x00FF0000)
    val += ((registers_values[1] << 24) & 0xFF000000)
    SetVar(rcrd_used_liters, val)
}
