...
main()
{
    ...
    new limit_type = GetVar(limit_type)
	
	// Set limit
	new val = 0x00000000;
    val += ((limit_type & 0x000000FF) << 8)
    val += ((limit_type & 0x0000FF00) >> 8)
	val = ((val) & 0xFFFF)
	new addr1 = (MODBUS_REGISTER_CARDS_IDX + 4 * cards_count + 2 * GetVar(card_id))
	Diagnostics("COMMAND: modbus write (%d) - %X", addr1, val) 
    modbus_master_preset_single_register(1, addr1, val)
