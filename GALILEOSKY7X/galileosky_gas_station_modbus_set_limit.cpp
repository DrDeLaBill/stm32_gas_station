...
main()
{
    ...
    new card_limit = GetVar(card_limit)
	
	// Set limit
	new val_l = 0x00000000;
    val_l += ((card_limit & 0x000000FF) << 8)
    val_l += ((card_limit & 0x0000FF00) >> 8)
	val_l = ((val_l) & 0xFFFF)
	new addr1 = (MODBUS_REGISTER_CARDS_IDX + 2* cards_count + 2 * GetVar(card_id))
	Diagnostics("COMMAND: modbus write (%d) - %X", addr1, val_l) 
    modbus_master_preset_single_register(1, addr1, val_l)
    
    new val_h = 0x000000000;
    val_h += ((card_limit & 0x00FF0000) << 8)
    val_h += ((card_limit & 0xFF000000) >> 8)
	val_h = ((val_h >> 16) & 0xFFFF)
	new addr2 = (MODBUS_REGISTER_CARDS_IDX + 2* cards_count + 2 * GetVar(card_id)) + 1
