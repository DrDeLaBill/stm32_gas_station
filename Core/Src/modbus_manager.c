...
#define MODBUS_U8_ARRAY_REG_COUNT(arr)   (sizeof(arr) / 2 + sizeof(arr) % 2)

#define MODBUS_REGISTER_CF_ID_IDX        ((uint16_t)0)
#define MODBUS_REGISTER_DEVICE_ID_IDX    ((uint16_t)MODBUS_REGISTER_CF_ID_IDX + sizeof(settings.cf_id))
